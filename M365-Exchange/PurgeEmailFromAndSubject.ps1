...
$subject = Read-Host -Prompt 'Enter the Subject for ContentMatchQuery'

# Combine the From and Subject into the ContentMatchQuery
$contentMatchQuery = "From:" + $fromCommand + " AND Subject:" + $subject

# Bypass execution policy temporarily
Set-ExecutionPolicy Unrestricted -Scope Process -Force

# Install and import the required modules (using the latest version available)
Install-Module -Name ExchangeOnlineManagement -Force
Import-Module ExchangeOnlineManagement

# Connect to Exchange Online using OAuth 2.0 authentication
Connect-ExchangeOnline -UserPrincipalName $UPN -ShowProgress $true

# Connect to the Security & Compliance Center
Connect-IPPSSession

# Create a new compliance search with both From and Subject filters
New-ComplianceSearch -Name $complianceSearchName -ExchangeLocation all -ContentMatchQuery $contentMatchQuery
